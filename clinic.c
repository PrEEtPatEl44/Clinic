...
void removePatient(struct Patient patient[], int max) 
{
    ...
    if (findPatientIndexByPatientNum(pNumber, patient, max) != -1) 
    {
        ...
        do 
        {
            scanf(" %c%c", &selection, &ch);

            if ((selection == 'n' || selection == 'N') && (ch == '\n')) 
            {
                printf("Operation aborted.\n\n");
                flag = 0;
                clearInputBuffer();
            }
            else if ((selection == 'y' || selection == 'Y') && (ch == '\n')) 
            {
...
void addAppointment(struct Appointment* app, int maxAppointments, struct Patient* pt, int maxPatients) 
{
    ...
    if (index >= 0) 
    {
        while (slotNotAvailable)
        {
            ...
            if (availableSlot(date, time, app, maxAppointments)) 
            {
                printf("\nERROR: Appointment timeslot is not available!\n\n");
            }
            else 
            {
                while ((time.hour < START_HOUR || time.hour > END_HOUR) || (time.hour == END_HOUR && time.min > 0) || (time.min % INTERVAL != 0)) 
                {
        ...
        index = nextAvailableSlot(app, maxAppointments);
        app[index].date = date;
        app[index].time = time;
...
void removeAppointment(struct Appointment* app, int maxAppointments, struct Patient* pt, int maxPatients) 
{
    ...
    if (index >= 0) 
    {
        ...
        aptIndex = checkValidity(patientNumber, date, app, maxAppointments);
...
int availableSlot(struct Date date, struct Time time, struct Appointment* app, int maxAppointments)
{
...
int nextAvailableSlot(struct Appointment* app, int maxAppointments)
{
    int i = 0, available = 0;
...
int checkValidity(int patientNumber, struct Date date, struct Appointment* app, int maxAppointments)
{
    int i = 0, valid = 0;
...
void inputPhoneNum(char* str, int minChar, int maxChar) {
    int flag = 1;
    char ch = 'a';
